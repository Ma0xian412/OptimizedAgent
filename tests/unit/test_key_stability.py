...
class TestRunKeyStability:
    def test_deterministic(self) -> None:
        ...
        spec = make_spec()
        rs = RunSpec(job=Job(command=["python"], args=["--x=1.0"]), result_path="/tmp/r.json")
    ...
    def test_different_config_different_key(self) -> None:
        ...
        spec = make_spec()
        k1 = builder.build(
            RunSpec(job=Job(command=["python"], args=["--x=1.0"]), result_path="/tmp/r1.json"),
            spec,
            "ds_v1",
        )
        k2 = builder.build(
            RunSpec(job=Job(command=["python"], args=["--x=2.0"]), result_path="/tmp/r2.json"),
            spec,
            "ds_v1",
        )
        assert k1 != k2
